/*
 * Qt4 securealumnicoin GUI.
 *
 * W.J. van der Laan 2011-2012
 * The Securealumnicoin Developers 2011-2012
 */

#include "securealumnicoingui.h"
...
const QString SecurealumnicoinGUI::DEFAULT_WALLET = "~Default";

SecurealumnicoinGUI::SecurealumnicoinGUI(bool fIsTestnet, QWidget *parent) :
    QMainWindow(parent),
    ...
    prevBlocks(0)
{
    ...
    if (!fIsTestnet)
    {
        ...
        QApplication::setWindowIcon(QIcon(":icons/securealumnicoin"));
        setWindowIcon(QIcon(":icons/securealumnicoin"));
    }
    else
    {
        setWindowTitle(tr("Securealumnicoin") + " - " + tr("Wallet") + " " + tr("[testnet]"));
        QApplication::setWindowIcon(QIcon(":icons/securealumnicoin_testnet"));
        setWindowIcon(QIcon(":icons/securealumnicoin_testnet"));
    }
#else
    ...
    if (!fIsTestnet)
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/securealumnicoin"));
    else
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/securealumnicoin_testnet"));
#endif
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void SecurealumnicoinGUI::createActions(bool fIsTestnet)
{
  ...
  //  connect(ui->bNewMessage, SIGNAL(clicked()), this, SLOT(showNormalIfMinimized()));
    ...
    if (!fIsTestnet)
        aboutAction = new QAction(QIcon(":/icons/securealumnicoin"), tr("&About Securealumnicoin"), this);
    else
        aboutAction = new QAction(QIcon(":/icons/securealumnicoin_testnet"), tr("&About Securealumnicoin"), this);
    ...
    if (!fIsTestnet)
        toggleHideAction = new QAction(QIcon(":/icons/securealumnicoin"), tr("&Show / Hide"), this);
    else
        toggleHideAction = new QAction(QIcon(":/icons/securealumnicoin_testnet"), tr("&Show / Hide"), this);
...
void SecurealumnicoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void SecurealumnicoinGUI::createToolBars()
{
...
void SecurealumnicoinGUI::setClientModel(ClientModel *clientModel)
{
...
void SecurealumnicoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void SecurealumnicoinGUI::createTrayIcon()
{
#ifndef Q_OS_MAC
...
void SecurealumnicoinGUI::createTrayIconMenu()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SecurealumnicoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SecurealumnicoinGUI::saveWindowGeometry()
{
    QSettings settings;
...
void SecurealumnicoinGUI::restoreWindowGeometry()
{
    QSettings settings;
...
void SecurealumnicoinGUI::optionsClicked()
{
...
void SecurealumnicoinGUI::aboutClicked()
{
...
void SecurealumnicoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
...
void SecurealumnicoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
    ...
    if(secs < 90*60 && count >= nTotalBlocks)
    {
		...
		labelWorldIcon->setPixmap(QPixmap(":/res/world_icon_on.png"));
...
void SecurealumnicoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
...
void SecurealumnicoinGUI::changeEvent(QEvent *e)
{
...
void SecurealumnicoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void SecurealumnicoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        "which goes to the nodes that process your transaction and helps to support the network. "
        "Do you want to pay the fee?").arg(SecurealumnicoinUnits::formatWithUnit(SecurealumnicoinUnits::SAC, nFeeRequired));
...
//void SecurealumnicoinGUI::incomingTransaction(const QString& date, int unit, qint64 amount, const QString& type, const QString& address)
void SecurealumnicoinGUI::incomingTransaction(const QModelIndex& parent, int start, int /*end*/)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(SecurealumnicoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}

void SecurealumnicoinGUI::gotoOverviewPage()
{
...
void SecurealumnicoinGUI::gotoHistoryPage()
{
...
void SecurealumnicoinGUI::gotoAddressBookPage()
{
...
void SecurealumnicoinGUI::gotoReceiveCoinsPage()
{
...
//void SecurealumnicoinGUI::gotoSendCoinsPage(QString addr)
void SecurealumnicoinGUI::gotoSendCoinsPage()
{
...
void SecurealumnicoinGUI::gotoSignMessageTab(QString addr)
{
...
void SecurealumnicoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void SecurealumnicoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SecurealumnicoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SecurealumnicoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
void SecurealumnicoinGUI::resizeEvent(QResizeEvent *e)
{
    updateMask();
    QMainWindow::resizeEvent(e);
}

void SecurealumnicoinGUI::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
...
void SecurealumnicoinGUI::updateMask()
{
    // main form mask
    _mask = QBitmap(size());
    _mask.clear();
...
void SecurealumnicoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void SecurealumnicoinGUI::menuFileRequested()
{
    QMenu menu(this);
...
void SecurealumnicoinGUI::menuOperationsRequested()
{
    QMenu menu(this);
    QAction* send = menu.addAction(tr("Send Securealumnicoins"));
    QAction* receive = menu.addAction(tr("Receive Securealumnicoins"));
...
void SecurealumnicoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SecurealumnicoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void SecurealumnicoinGUI::backupWallet()
{
...
void SecurealumnicoinGUI::changePassphrase()
{
...
void SecurealumnicoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void SecurealumnicoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void SecurealumnicoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SecurealumnicoinGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void SecurealumnicoinGUI::onMiningClicked()
{
    //ui->wMining->setStyleSheet("background-color:black;");
    centralWidget->setCurrentWidget(miningPage);
}

void SecurealumnicoinGUI::on_bHelp_clicked()
{    
    rpcConsole->show();
}
