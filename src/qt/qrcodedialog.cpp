...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SAC unit input in lnReqAmount, we generate the URI with SAC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(SecurealumnicoinUnits::format(SecurealumnicoinUnits::SAC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
