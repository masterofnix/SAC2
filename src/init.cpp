...
void Shutdown()
{
    ...
    RenameThread("securealumnicoin-shutoff");
    nTransactionsUpdated++;
    StopRPCThreads();
    ShutdownRPCMining();
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateSecurealumnicoins(false, NULL);
    StopNode();
    {
        LOCK(cs_main);
        if (pwalletMain)
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/securealumnicoin.conf are parsed in qt/securealumnicoin.cpp's main()
        ...
        {
            // First part of help message is specific to securealumnicoind / RPC client
...
int main(int argc, char* argv[])
{
    ...
    // Connect securealumnicoind signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 40331 or testnet: 40329)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 40334 or testnet: 40333)") + "\n" +
...
{
    RenameThread("securealumnicoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Securealumnicoin process is using the data directory.
    ...
    // requested to kill securealumnicoin-qt during the last operation. If so, exit.
    ...
    if (pwalletMain)
        GenerateSecurealumnicoins(GetBoolArg("-gen", false), pwalletMain);
